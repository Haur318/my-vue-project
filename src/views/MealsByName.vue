    ...
    <div :class="filteredMeals.length == 0 ? 'md:grid md:place-items-center' : ''">
      <div v-if="filteredMeals.length > 0" class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-5 pt-2">
        <div v-for="filteredMeal in filteredMeals" :key="filteredMeal" class="shadow rounded-xl">
          <router-link :to="{ name: 'mealDetails', params: { id: filteredMeal.idMeal } }">
            <img
          ...
          <div
            class="px-4 pt-3 pb-3 bg-gradient-to-r from-yellow-500 to-amber-600 text-white rounded-b-lg"
          >
      ...
      <div
        v-else
        class="grid place-items-center bg-white md:w-2/4 mt-8 rounded-lg"
        data-aos="fade-down"
      >
